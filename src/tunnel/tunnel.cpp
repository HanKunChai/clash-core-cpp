...
std::shared_ptr<adapter::ProxyAdapter> Tunnel::match(const constant::Metadata& metadata)
{
    ...
    for (const auto& rule : rules_)
    {
        if (rule->match(metadata))
        {
            ...
            LOG_DEBUG("Match rule: %s -> %s", rule->payload().c_str(), adapterName.c_str());
            ...
            else
            {
                ...
                LOG_WARN("Proxy not found: %s, using DIRECT", adapterName.c_str());
                return proxies_["DIRECT"];
            }
        }
    }
    ...
    LOG_DEBUG("No rule matched, using DIRECT");
    return proxies_["DIRECT"];
}
