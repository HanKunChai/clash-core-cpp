    ...
    void start()
    {
        ...
        resolver_.async_resolve(option_.server, std::to_string(option_.port),
            [this, self](const std::error_code& ec, asio::ip::tcp::resolver::results_type results)
            {
                ...
                {
                    log::error("Shadowsocks resolve error: {}", ec.message());
                    handler_(ec, nullptr);
                }
            });
    }
...
private:
    ...
    void connect(const asio::ip::tcp::resolver::results_type& endpoints)
    {
        ...
        asio::async_connect(socket_, endpoints,
            [this, self](std::error_code ec, asio::ip::tcp::endpoint /*endpoint*/)
            {
                ...
                {
                    log::error("Shadowsocks connect error: {}", ec.message());
                    handler_(ec, nullptr);
                }
            });
    }
    ...
    void handshake()
    {
        ...
        {
            log::error("Unsupported cipher: {}", option_.cipher);
        ...
        conn_ptr->async_write(asio::buffer(target),
            [this, conn_holder](std::error_code ec, size_t) mutable
            {
                if (ec)
                {
                    log::error("Shadowsocks handshake write error: {}", ec.message());
                    handler_(ec, nullptr);
                }
                else
                {
                    log::debug("Shadowsocks handshake success");
