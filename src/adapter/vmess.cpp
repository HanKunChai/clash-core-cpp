...
private:
    ...
    void resolve_proxy()
    {
        ...
        resolver_.async_resolve(option_.server, std::to_string(option_.port),
            [this, self](std::error_code ec, asio::ip::tcp::resolver::results_type results)
            {
                ...
                {
                    log::error("VMess resolve error: {}", ec.message());
                    handler_(ec, nullptr);
                }
            });
    }
    ...
    void connect_proxy(const asio::ip::tcp::resolver::results_type& endpoints)
    {
        ...
        asio::async_connect(socket_, endpoints,
            [this, self](std::error_code ec, asio::ip::tcp::endpoint /*endpoint*/)
            {
                ...
                {
                    log::error("VMess connect error: {}", ec.message());
                    handler_(ec, nullptr);
                }
            });
    }
    ...
    void handshake()
    {
        log::debug("VMess connected to server {}:{}", option_.server, option_.port);
        log::warn("VMess protocol not implemented yet, acting as plain TCP tunnel (will likely fail)");
