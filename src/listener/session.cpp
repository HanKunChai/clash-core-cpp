...
void Session::handle_http_request(std::size_t length)
{
    ...
    LOG_INFO("HTTP Request to %s:%d", host.c_str(), port);
...
void Session::handle_socks5_handshake(std::size_t length)
{
    ...
    if (length < 2)
    {
        LOG_WARN("Socks5 handshake too short");
        return;
    }
    ...
    if (buffer_[0] != 0x05)
    {
        LOG_WARN("Invalid Socks5 version: %d", (int)buffer_[0]);
        return;
    }
...
void Session::handle_socks5_request(std::size_t length)
{
    ...
    if (length < 4)
    {
        LOG_WARN("Socks5 request too short");
        return;
    }
    ...
    if (buffer_[1] != 0x01)
    { 
        LOG_WARN("Unsupported Socks5 command: %d", (int)buffer_[1]);
        // TODO: 应该回复不支持的命令错误
        return;
    }
    ...
    { // IPv4
        if (length < 10)
        {
            LOG_WARN("Socks5 request too short for IPv4");
            return;
        }
    ...
    { // Domain
        ...
        {
            LOG_WARN("Socks5 request too short for Domain");
            return;
        }
    ...
    { // IPv6
        if (length < 22)
        {
            LOG_WARN("Socks5 request too short for IPv6");
            return;
        }
    ...
    {
        LOG_WARN("Unsupported Socks5 address type: %d", addr_type);
        return;
    }

    LOG_INFO("Socks5 Connect Request to %s:%d", host.c_str(), port);
