...
{
    LOG_INFO("TCP Listener started on port %d", port_);
    do_accept();
}
...
{
    acceptor_.close();
    LOG_INFO("TCP Listener on port %d closed", port_);
}
    ...
    acceptor_.async_accept(
        [this](std::error_code ec, asio::ip::tcp::socket socket)
        {
            if (!ec)
            {
                try
                {
                    LOG_DEBUG("New connection from %s", socket.remote_endpoint().address().to_string().c_str());
                    // 创建并启动 Session
                ...
                {
                    LOG_ERROR("Error creating session: %s", e.what());
                }
            }
            else
            {
